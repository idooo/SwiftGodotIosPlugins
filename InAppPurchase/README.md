...
func _ready() -> void:
	if _inapppurchase == null && ClassDB.class_exists("InAppPurchase"):
		...
		_inapppurchase.in_app_purchase_fetch_transactions.connect(_on_in_app_purchase_fetch_transactions)
...
func _on_in_app_purchase_fetch_transactions(transactions: Array[Dictionary]) -> void:
...
## Signals
- `in_app_purchase_fetch_success` SignalWithArguments\<GArray>
- `in_app_purchase_fetch_error` SignalWithArguments\<Int, String>
...
- `in_app_purchase_error` SignalWithArguments\<Int, String>
- `in_app_purchase_restore_success` SignalWithArguments\<GArray>
- `in_app_purchase_restore_error` SignalWithArguments\<Int, String>
...
- `purchaseProduct(productID: String, appAccountToken: String = "")` - Purchase a given product. Optionally pass a UUID string to link the transaction to a user account for server-side validation.
